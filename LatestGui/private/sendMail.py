...
while True:
    ...
    for row in rows:
        ...
        if row.Status=='3':
            message='Hi  '+ row.UserEmail.split('@')[0]+'\n    Your test job for url '+ row.Url +' was completed successfully. You can view the report by clicking the following link: http://10.1.33.107:8000/FunkLoad/default/test/'+str(row.id)+'  .\n\n\nCheers\nTeam FunkloadApp \n'
